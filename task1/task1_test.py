...
def nonconstant_features(X):
  return np.array([len(set(v)) != 1 for v in X.T])


j_geo = np.array(j_geo)
j_geo_mask = nonconstant_features(j_geo)
official = np.vstack([j_geo, j_rot])
print(official.T[j_geo_mask].T)
print(sim.jacobianMatrix('LARM_JOINT5'))

print("Is transisition close ",np.isclose(j_geo.T[j_geo_mask].T, sim.jacobianMatrix('LARM_JOINT5')[:3,:]).all())
